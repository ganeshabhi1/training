...
public class TaskSystemImpl implements TasksSystemDao {
	...
	public int addnote(int taskid, String notes) {
		//establishing connetion
				...
				try {
					//preparing sql statement to insert values into database
					String sql = "update taskde set notes=? where taskid=?";
					PreparedStatement st = c.prepareStatement(sql);
					st.setInt(2, taskid);
	...
	public boolean addUser(UserBeen user) throws SQLException {
		...
		String sql = "insert into userdetails1 (id ,firstname ,lastname ,password ) values(?,?,?,?)";
	...
	public List<UserBeen> getAllusers() {
		Connection c = mySQLConnectionimpl.getConnection();
		List<UserBeen> userlist= new ArrayList<UserBeen>();
		try {
			//preparing sql statement to insert values into database
			...
			ResultSet rs =st.executeQuery();
			while(rs.next()) {
				UserBeen task1 = new UserBeen();
				task1.setId(rs.getInt("id"));
				task1.setFirstname(rs.getString("firstname"));
				task1.setLastname(rs.getString("lastname"));
				task1.setPassword(rs.getString("password"));
